...
from .config import Config, VALID_LAYOUTS, VALID_DIAGRAM_MODES, VALID_SPACINGS
...
def run_report_all(cfg: Config) -> None:
    ...
    log.info(
        "report-all: generating %d combinations into %s",
        n_combos, variants_root,
    )
    ...
    for layout, diagram_mode, spacing in product(
        sorted(VALID_LAYOUTS), sorted(VALID_DIAGRAM_MODES), sorted(VALID_SPACINGS)
    ):
        label = f"{_safe_layout_name(layout)}_{diagram_mode}_{spacing}"
        combo_dir = variants_root / label
        combo_dir.mkdir(parents=True, exist_ok=True)
        (combo_dir / "graph.json").write_text(json.dumps(graph))
        ...
        try:
            generate_drawio(combo_cfg)
            generate_docs(combo_cfg)
            succeeded += 1
            log.info("  OK   %s", label)
        except Exception:
            failed += 1
    ...
    lines = [
        f"# Architecture Diagram Variants \u2014 {cfg.app}",
        "",
        f"> Generated by `python3 -m tools.azdisc report-all config.json`.",
        f"> {n_combos} combinations: {len(VALID_LAYOUTS)} layouts"
        f" \u00d7 {len(VALID_DIAGRAM_MODES)} diagram modes"
        f" \u00d7 {len(VALID_SPACINGS)} spacing presets.",
        "",
    ]

    for layout, diagram_mode, spacing, label in combos:
        lines.append("---")
        lines.append("")
        lines.append(f"## {layout} \u2014 {diagram_mode} \u2014 {spacing}")
        lines.append("")
        lines.append(
            f"**Layout:** `{layout}` | **Mode:** `{diagram_mode}` | **Spacing:** `{spacing}`"
        )
        lines.append("")
        png_rel = f"{label}/diagram.png"
        if (variants_root / png_rel).exists():
            lines.append(f"![{layout} {diagram_mode} {spacing}]({png_rel})")
            lines.append("")
        lines.append(f"[diagram.drawio]({label}/diagram.drawio)")
        lines.append("")

    report_path.write_text("\n".join(lines))
    log.info("report-all: wrote %s", report_path)

    log.info(
        "report-all complete: %d/%d succeeded, %d failed",
        succeeded, succeeded + failed, failed,
    )
    if failed:
        raise SystemExit(f"report-all: {failed} combination(s) failed")
